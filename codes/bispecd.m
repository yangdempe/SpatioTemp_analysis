function [Bspec,waxis] = bispecd (y,  nfft, wind, nsamp, overlap,Fs,fmax)
...
% ---------------- set decimal and output vector -------------------
fs=Fs/nsamp;
m=round(fmax/fs);
waxis = (-m:m)'*fs;
n=2*m+1;

w=hanning(nfft);    % Window data
Nw=sqrt(w'*w*Fs);
% ---------------- accumulate triple products ----------------------
Bspec    = zeros(n,n);

mask = hankel(1:n,[n,1:n-1] );   % the hankel mask (faster)
locseg = (1:nsamp)';
for krec = 1:nrecs
    xseg   = y(locseg);
    Xf     = fft((xseg-mean(xseg)).*w)/Nw;
    Xf     = Xf([nsamp-m+1:nsamp,1:m+1]);
    CXf    = conj(Xf);
    Bspec  = Bspec + (Xf * Xf.') .* ...
                     reshape(CXf(mask), n, n);
...
if (winsize > 1)
    ...
    Bspec = Bspec(lby2+1:lby2+n,lby2+1:lby2+n);
end

% ------------ contour plot of magnitude bispectum --------------------
%figure('visible','off')
%contourf(waxis(m:n),waxis(m:n),abs(Bspec(m:n,m:n)),50,'LineStyle','none'), grid on
%colorbar
%title('Bispectrum via dFFT')
%xlabel('f1 (Hz)'),   ylabel('f2 (Hz)')
%xlim([0,fmax]), ylim([0,fmax])
%set(gca,'clim',[0 30],'fontsize',20)
return
